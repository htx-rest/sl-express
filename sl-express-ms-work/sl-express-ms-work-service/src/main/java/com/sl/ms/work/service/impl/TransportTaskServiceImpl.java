 ...
 */
@Service
public class TransportTaskServiceImpl extends ServiceImpl<TransportTaskMapper, TransportTaskEntity> implements TransportTaskService {
    ...
    public List<String> queryTransportOrderIdListById(Long id) {
        //通过运输任务找到运单id列表
        LambdaQueryWrapper<TransportOrderTaskEntity> queryWrapper = new LambdaQueryWrapper<TransportOrderTaskEntity>().eq(TransportOrderTaskEntity::getTransportTaskId, id);
        List<TransportOrderTaskEntity> orderList = this.transportOrderTaskService.list(queryWrapper);
        if (CollUtil.isEmpty(orderList)) {
            return Collections.emptyList();
        }
        //运单id列表
        return orderList.stream().map(TransportOrderTaskEntity::getTransportOrderId).collect(Collectors.toList());
    }
    ...
    public List<TransportTaskStatusCountDTO> groupByStatus() {
        //将所有的枚举状态放到集合中
        List<TransportTaskStatusCountDTO> statusCountList = Arrays.stream(TransportTaskStatus.values()).map(status -> TransportTaskStatusCountDTO.builder().status(status).statusCode(status.getCode()).count(0L).build()).collect(Collectors.toList());
    ...
    public void adjust(TaskTransportUpdateDTO dto) {
        if (ObjectUtil.hasEmpty(dto, dto.getId()) || ObjectUtil.isAllEmpty(dto.getTransportTripsId(), dto.getTruckId(), dto.getDriverId())) {
    ...
    public void delayedDelivery(TransportTaskDelayDeliveryDTO transportTaskDelayDeliveryDTO) {
        ...
        updateWrapper.eq(ObjectUtil.isNotEmpty(transportTaskDelayDeliveryDTO.getTransportTaskId()), TransportTaskEntity::getId, transportTaskDelayDeliveryDTO.getTransportTaskId()).set(ObjectUtil.isNotEmpty(delayTime), TransportTaskEntity::getPlanDepartureTime, delayTime).set(ObjectUtil.isNotEmpty(transportTaskDelayDeliveryDTO.getDelayReason()), TransportTaskEntity::getMark, transportTaskDelayDeliveryDTO.getDelayReason());
    ...
    public List<Long> findByAgencyId(Long startAgencyId, Long endAgencyId) {
        //1.构造查询条件
        LambdaQueryWrapper<TransportTaskEntity> queryWrapper = Wrappers.<TransportTaskEntity>lambdaQuery().eq(ObjectUtil.isNotEmpty(startAgencyId), TransportTaskEntity::getStartAgencyId, startAgencyId).eq(ObjectUtil.isNotEmpty(endAgencyId), TransportTaskEntity::getEndAgencyId, endAgencyId);

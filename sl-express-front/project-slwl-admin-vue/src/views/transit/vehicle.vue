...
export default {
  ...
  computed: {
    // status() 是一个计算属性，它的值依赖于 data 中的 workStatus，当 workStatus 发生变化时，status 也会更新，这里的 status 也是一个响应式的数据，可以在模板中使用，也可以在计算属性中使用，也可以在方法中使用，也可以在 watch 中使用，也可以在生命周期中使用
    status() {
      return status
    }
  },
  ...
  methods: {
    ...
    async getCarTypeStatics() {
      ...
      this.notUseNum = res[0] || 0
      this.useNum = res[1] || 0
      this.allNum = Number(this.notUseNum) + Number(this.useNum)
    },
    // 状态变更

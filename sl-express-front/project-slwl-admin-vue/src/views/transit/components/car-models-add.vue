    ...
    >
      ...
      >
        ...
        <!-- <el-form-item
          ...
          prop="goodsTypes"
        >
          <template v-if="titleInfo.text === '新增车型'">
            <el-select
              ref="selectInput"
              v-model="formBase.goodsTypes"
              value-key="id"
              placeholder="请选择适用货品类型"
              clearable
              style="width: 100%"
              multiple
            >
          ...
          <template v-if="titleInfo.text === '编辑车型'">
            <el-input
              v-if="!selectShow"
              v-model="getallGoodsTypes"
              suffix-icon="el-icon-arrow-down"
              @focus="getSelectShowFn"
            />
            <el-select
              v-if="selectShow"
              ref="selectInput"
...
export default {
  ...
  computed: {
    getallGoodsTypes() {
      return this.formBase.goodsTypes.map((obj) => obj.name).join('、')
    }
  },
  // 组件更新
  updated: function () {},

            ...
            <el-row :gutter="24">
              ...
              <!-- <el-col :span="12">
                <el-form-item label="外廓尺寸：" prop="outsideDimensions">
                  <el-input v-model="vehiclesDriving.outsideDimensions" :disabled="disabledStr" maxlength="20"></el-input>
                </el-form-item>
              </el-col> -->
              <el-col
                :span="8"
              ...
              >
                <el-form-item
                  ...
                  label-width="100px"
                >
                  <!-- <el-input v-model="vehiclesDriving.validityPeriod" :disabled="disabledStr" maxlength="20"></el-input> -->
                  <el-date-picker
  ...
  data() {
    return {
      ...
      rules: {
        ...
        //         const reg = /^[0-12]+$/
        //         if (!reg.test(value)) {
        //           callback(new Error('只能输入整数，请重新输入'))
        //         } else {
        //           callback()
        //         }
        //       } else {
  ...
  methods: {
    ...
    getUrl() {
      return process.env === 'development'
        ? '/api1/files/imageUpload'
        : 'http://slwl-geteway-t.itheima.net/manager/files/imageUpload'
    },
    goBack() {},
    // 上传头像
    handleAvatarSuccess(res, file) {
      const _this = this
      if (res.code === 200) {
        _this.vehiclesDriving.picture = res.data
      }
    },
    // 获取车辆详情-行驶证信息
    async getList(id) {
      ...
      this.vehiclesDriving.picture = res.picture
        ...
        : ''
      // this.$set(this.vehiclesDriving.picture, 1, res.picture.split(',').map((item) => {
      //   return { url: item }
      // }))

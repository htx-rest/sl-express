...
export default {
  ...
  methods: {
    ...
    handleAddCourier(rows) {
      ...
      if (length >= 2) {
        this.$message({
          message: '不能同时多个作业进行分配！',
          center: true
        })
        return
      }
      this.couriAddForm.pickupId = rows[0].id
      this.dialogFormVisible = true
      this.titleInfo.text = '分配快递员'
    },
    // 弹层隐藏
    dialogFormH() {
      this.dialogFormVisible = false
    ...
    createData() {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          ...
          const data = {
            courier: {
              userId: this.couriAddForm.courierName
            }
          }
          // eslint-disable-next-line no-undef
          await pickupTaskUpdate(this.couriAddForm.pickupId, data).then(
            (res) => {
              //  && String(res.data.code) === '0')
              // if (String(res.status) === '200') {
              // this.$message.success('分配快递员成功')

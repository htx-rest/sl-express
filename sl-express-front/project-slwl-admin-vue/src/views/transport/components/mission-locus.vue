...
export default {
  ...
  methods: {
    ...
    handleSearchComplete(res) {
      this.polylinePath = res.getPlan(0).getRoute(0).getPath()
    },
    goBack() {},
    // 获取运输详情-任务轨迹信息
    async getList(id) {
      ...
      this.missionLocus.driverName = res.drivers.map((item) => item.name)
      // if (res.drivers != null) {
      //   // this.missionLocus.driverName = res.drivers.name
      //   this.missionLocus.driverName = res.driverName
      // }
      ...
      // 查询轨迹
      this.queryParams.transportTaskId = id
      // const { data: result } = await transportLocus(this.queryParams)
      // this.polylinePath = result.data.polyLinePath
      // 查询途径垫
      // const { data: resultPoint } = await transportPoint(id)
      // this.pointsArr = resultPoint
    }
  }
}
</script>

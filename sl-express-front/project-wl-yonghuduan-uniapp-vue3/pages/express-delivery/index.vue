...
<script setup>
	...
	onLoad((options) => {
		...
		// 第一次进入寄件,回退时
		if (options.id) {
			searchAddressDetail(options.id, options.type)
		}
		...
		//如果从物品信息页面返回到当前页面需要重新获取之前的地址信息
		if (options.isFromGoods) {
			if (uni.getStorageSync('sendId')) {
				searchAddressDetail(uni.getStorageSync('sendId'), 'send')
			}
			if (uni.getStorageSync('getId')) {
	...
	const order = () => {
		...
		doOrder({
			...
			totalVolume: goods.volume ? goods.volume : 1,

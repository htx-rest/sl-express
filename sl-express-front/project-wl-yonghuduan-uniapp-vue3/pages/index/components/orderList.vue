<template>
	<view class="orderList">
		...
		<view v-else>
			...
			<!-- 		<scroll-view class="scrollView" :class="isShowMore?'maxHeight':''" scroll-y
				lower-threshold="30" @scrolltolower="LoadMoreCustomers"> -->
			<view class="order-item" v-for="(item,index) in allOrderList.data" :key="index"
				...
				<view class="order-detail">
					<!-- 已取件运输信息 -->
					<view v-if="item.status ===23001 " class="detail-item">
						<text>已取件：</text>{{ item.transportOrderPointVOS.length>0?item.transportOrderPointVOS[item.transportOrderPointVOS.length-1].info:''}}
					</view>
					<!-- 运输中信息 -->
					<view v-if="[23005,23008].includes(item.status) " class="detail-item">
						<text>运送中：</text>{{ item.transportOrderPointVOS.length>0?item.transportOrderPointVOS[item.transportOrderPointVOS.length-1].info:''}}
					</view>
					<!-- 已签收 -->
					<view v-if="item.status ===23009 " class="detail-item"><text>已签收：</text>{{ item.transportOrderPointVOS.length>0?item.transportOrderPointVOS[item.transportOrderPointVOS.length-1].info:''}}</view>
					<!-- 已拒收 -->
					<view v-if="item.status ===23010 " class="detail-item"><text>已拒收：</text>{{ item.transportOrderPointVOS.length>0?item.transportOrderPointVOS[item.transportOrderPointVOS.length-1].info:''}}</view>
					<!-- 预计上门时间 -->
				...
				<view class="btn-box">
					<!-- 待支付 -->
					<!-- 	<view class="wait-pay" v-if="item.status===21000">
							<view class="price">运费<text>¥68</text></view>
							<view class="btn">结算</view>
						</view> -->
					<view class="btn-list" @click.stop="handleSecondQi">
						<!-- <view class="btn" v-if='item.status===23000'>修改</view> -->
						<!-- <view class="btn" v-if='item.status===23008'>催单</view> -->
	...
	const getOrderListFunc = (flag) => {
		...
		getOrderList({
			...
			if (res.data) {
				...
				// if (flag === 'topPull') {
				// 	allOrderList.data = allOrderList.data.concat(res.data.items)
				// } else {
				// 	someOrderList.data = res.data.items.slice(0, 3)
				// 	allOrderList.data = res.data.items
				// }
				allOrderList.data = res.data.items.slice(0, 3)
			}

					...
					<view class="status">
						...
						<view class="status-tips">
							...
							<view class="timeing" v-else>
								...
								:''}}
								<!-- {{ new Date(orderInfo.estimatedStartTime.replace(/-/g, '/')).getMonth()+1 }} -->
								<!-- 预计5月18日 22:00前送达 -->
						...
						<view class="logistics-orderInfo-item" :class="[index===0?'active':'']" :key="index"
							...
							<view class="logistics-orderInfo-right">
								...
								<view class="desc" :class="index === 0 || item.status === 23010?'active':''"
									v-html="strInit(item.info)">{{item.info}}
								</view>
							</view>
						</view>
					</view>
...
<script setup>
	...
	const findPoint = () => {
		...
		console.log(distanceList, pointIndex, polyline.data, 'distanceList')
	}

		...
		<view class="container">
			<view class="delayedCont">
				<view class="lineBoder">
					...
					<uni-datetime-picker v-else type="datetime" v-model="exceptionsDeta.time" :clear-icon="false" />
				</view>
				<view class="lineBoder">
					<text>上报位置</text>
				...
				<view v-else class="lineBoder">
					<view class="" style="width: 100%;">
						<text>异常描述</text>
						<textarea class="textInput" v-model="formData.delayReason" placeholder-style="color:#818181" placeholder="请输入异常描述"/>
					</view>
				</view>
			</view>
			<view class="delayedCont">
				<view :class="{upPicCont:true, setBottom: type == 'details'}" >
					...
					<view v-if="type == 'details'" class="exceptionImages">
						<image class="exceptionImage" v-for="(item, index) in exceptionsDeta.images" :src="item" key="index" mode=""></image>
					</view>
					<uni-file-picker 
						v-else
						v-model="imageValue" 

...
import com.sl.ms.base.service.truck.*;
...
public class TruckPlanServiceImpl extends ServiceImpl<TruckPlanMapper, TruckPlanEntity>
    ...
    public List<TruckPlanDto> pullUnassignedPlan(Integer shardTotal, Integer shardIndex) {
        ...
        int time = Integer.parseInt(dispatchTime != null ? dispatchTime : "2");
    ...
    public void finishedPlan(Long currentOrganId, Long planId, Long truckId, StatusEnum statusEnum) {
        ...
        // 如果计划的车辆和登记的车辆不一致  解除车辆和车次的绑定关系 再次绑定触发新计划
//        if (ObjectUtil.notEqual(truckPlanEntity.getTruckId(), truckId)) {
            // 暂时不做处理 运输任务调整是临时性的
//            // 解除原有车次绑定关系
//            transportTripsTruckDriverService.delete(truckPlanEntity.getTransportTripsId(), truckPlanEntity.getTruckId());
//            // 解除原有司机绑定关系
//            truckDriverService.disableTruckId(truckPlanEntity.getTruckId());
//        }

...
public class WorkSchedulingServiceImpl extends ServiceImpl<WorkSchedulingMapper, WorkSchedulingEntity> implements WorkSchedulingService {
    ...
    public PageResponse<WorkSchedulingDTO> queryForPage(WorkSchedulingQueryDTO workSchedulingQueryDTO) {
        ...
        if (CollectionUtil.isEmpty(records)) { //有数据
            return PageUtils.toResponsePage(entityPage, WorkSchedulingDTO.class, null);
        }
        ...
        if (isQueryHistory) { //查询历史数据
            ...
            if (CollectionUtil.isNotEmpty(historyEntity)) {
                historyEntity.parallelStream().forEach(entity -> {
                    historyMap.put(entity.getUserId() + "" + entity.getWorkDay(), "");
                });
            }
        }
        ...
        return PageUtils.toResponsePage(entityPage, WorkSchedulingDTO.class, (entity, dto) -> {
    ...
    public void add(WorkSchedulingAddDTO workSchedulingAddDTO) {
        WorkSchedulingEntity workSchedulingEntity = ObjectUtils.convert(workSchedulingAddDTO, WorkSchedulingEntity.class, (dto, entity) -> {
    ...
    public void batchAdd(List<WorkSchedulingAddDTO> workSchedulingAddDTOList) {
        ...
        if (CollectionUtil.isEmpty(workSchedulingAddDTOList)) {
            return;
        }
        List<WorkSchedulingEntity> batchAddEntityList = new ArrayList<>();

        for (int count = 0; count < workSchedulingAddDTOList.size(); count++) {
            WorkSchedulingEntity workSchedulingEntity = ObjectUtils.convert(workSchedulingAddDTOList.get(count),
    ...
    public void update(WorkSchedulingUpdateDTO workSchedulingUpdateDTO) {
        ...
        WorkSchedulingEntity workSchedulingEntity = ObjectUtils.convert(workSchedulingUpdateDTO,
                WorkSchedulingEntity.class, (dto, entity) -> {
                    entity.setUpdated(LocalDateTime.now());
                    entity.setUpdater(dto.getOperator());
    ...
    public List<WorkSchedulingDTO> monthSchedule(List<Long> userIds, Long agencyId, Byte type, LocalDateTime time) {
        if (CollectionUtil.isEmpty(userIds) && agencyId == null) {
            return null;
        }

        QueryWrapper<WorkSchedulingEntity> queryWrapper = new QueryWrapper<>();
        queryWrapper
                .in(CollUtil.isNotEmpty(userIds), "user_id", userIds)
                .eq(ObjectUtil.isNotEmpty(agencyId), "agency_id", agencyId)
                .eq("user_type", type);
    ...
    private WorkSchedulingDTO appenWorkPatten(WorkSchedulingDTO workSchedulingDTO, Map<Long, WorkPatternEntity> workPatternEntityMap) {
        if (CollectionUtil.isNotEmpty(workPatternEntityMap)) {
    ...
    private WorkSchedulingDTO parse(WorkSchedulingEntity entity, LocalDateTime time) {
        ...
        return ObjectUtils.convert(entity, WorkSchedulingDTO.class, (workSchedulingEntity, workSchedulingDTO) -> {
            //暂未排班的情况设置为空
